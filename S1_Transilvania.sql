...
CREATE TABLE adresa_1 (
    ...
    numar VARCHAR2(20),
    CONSTRAINT fk_adresa_oras FOREIGN KEY (id_oras) REFERENCES oras_1(id_oras)
);
...
CREATE TABLE sala_1 (
    ...
    CONSTRAINT fk_sala_adresa FOREIGN KEY (id_adresa) REFERENCES adresa_1(id_adresa),
    CONSTRAINT fk_sala_plan FOREIGN KEY (denumire_plan) REFERENCES plan(denumire_plan)
);

CREATE TABLE departament_1 (
    ...
    CONSTRAINT fk_dep_sala FOREIGN KEY (id_sala) REFERENCES sala_1(id_sala),
...
CREATE TABLE angajat_v2_1 (
    id_angajat NUMBER PRIMARY KEY,
    id_departament NUMBER NOT NULL,
    functie VARCHAR2(50) NOT NULL,
    salariu NUMBER(8,2) NOT NULL CHECK (salariu > 0),
    CONSTRAINT fk_ang2_ang1 FOREIGN KEY (id_angajat) REFERENCES angajat_v1_1(id_angajat),
...
CREATE TABLE clase_1 (
    id_clasa NUMBER PRIMARY KEY,
    denumire VARCHAR2(100) NOT NULL,
    categorie VARCHAR2(50),
    id_sala NUMBER NOT NULL,
    CONSTRAINT fk_clase_sala FOREIGN KEY (id_sala) REFERENCES sala_1(id_sala)
);

CREATE TABLE eveniment_1 (
    id_eveniment NUMBER PRIMARY KEY,
    id_sala NUMBER NOT NULL,
    nume VARCHAR2(100) NOT NULL,
    data DATE,
    numar_bilete NUMBER CHECK (numar_bilete > 0),
    CONSTRAINT fk_ev_sala FOREIGN KEY (id_sala) REFERENCES sala_1(id_sala)
);

CREATE TABLE bilet_1 (
    id_bilet NUMBER PRIMARY KEY,
    id_eveniment NUMBER NOT NULL,
    pret NUMBER(6,2) CHECK (pret > 0),
    id_client NUMBER NOT NULL,
    CONSTRAINT fk_bilet_ev FOREIGN KEY (id_eveniment) REFERENCES eveniment_1(id_eveniment),
    CONSTRAINT fk_bilet_client FOREIGN KEY (id_client) REFERENCES client(id_client)
);

CREATE TABLE produs_1 (
    ...
    id_sala NUMBER NOT NULL,
    id_client NUMBER, 
    CONSTRAINT fk_prod_sala FOREIGN KEY (id_sala) REFERENCES sala_1(id_sala),
    CONSTRAINT fk_prod_client FOREIGN KEY (id_client) REFERENCES client(id_client)
);

CREATE TABLE vinde_1 (
    id_sala NUMBER NOT NULL,
    id_abonament NUMBER NOT NULL,
    CONSTRAINT fk_vinde_sala_1 FOREIGN KEY (id_sala) REFERENCES sala_1(id_sala)
);
...
INSERT INTO vinde_1 (id_sala, id_abonament) VALUES (4, 1002);
